...
def augmentate(path):
    ...
    out_images= {"":img,"flipped":flipped_img,
...
def save_images(images, img_name):
    for augmentation_type, img in images.items():
        ...
        cv2.imwrite(img_name+"_"+augmentation_type+".jpeg",img)
...
if __name__ == "__main__":
    ...
    for idx in range(len(img_labels)):
        ...
        save_images(images, "augmented_dataset_test2/"+ str(img_path).split('/')[-1])
        label = img_labels.iloc[idx, 1]
        for augmentation_type, img in images.items():
            out_labels = out_labels.append({"img": img_labels.iloc[idx, 0]+"_"+augmentation_type+".jpeg", "success": label, "failure":img_labels.iloc[idx, 2]},ignore_index=True)
        


    out_labels.to_csv("out.csv", index=False)
